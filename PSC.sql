...
DELIMITER //
CREATE PROCEDURE get_last_record_identificator(IN p_table VARCHAR(20), IN p_column VARCHAR(20), OUT p_result VARCHAR(45))
DETERMINISTIC
BEGIN

SET @query =CONCAT('SELECT MAX(',p_column, ') AS last_val INTO @outvar FROM ',p_table );
...
#CALL get_last_record_identificator("narudzbenica","broj_narudzbe",@rijesenje);
#SELECT @rijesenje as last_identificator;
#SELECT MAX(broj_narudzbe) AS last_identificator FROM narudzbenica;
